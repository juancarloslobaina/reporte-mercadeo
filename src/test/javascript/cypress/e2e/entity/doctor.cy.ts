...
describe('Doctor e2e test', () => {
  ...
  const doctorSample = { nombreDoctor: 'monitor' };
  ...
  describe('new Doctor page', () => {
    ...
    it('should create an instance of Doctor', () => {
      ...
      cy.get(`[data-cy="correoPersonal"]`).type('Aldea').should('have.value', 'Aldea');

      cy.get(`[data-cy="correoCorporativo"]`).type('Metal HTTP').should('have.value', 'Metal HTTP');

      cy.get(`[data-cy="telefonoPersonal"]`).type('Panamá Investment Falkland').should('have.value', 'Panamá Investment Falkland');

      cy.get(`[data-cy="telefonoCorporativo"]`).type('Iran Andalucía Actualizable').should('have.value', 'Iran Andalucía Actualizable');
