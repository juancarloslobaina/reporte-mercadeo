...
public interface ReporteRepository extends JpaRepository<Reporte, Long>, JpaSpecificationExecutor<Reporte> {
    ...
    @Query(
        value = "select distinct reporte from Reporte reporte left join fetch reporte.centro left join fetch reporte.doctor left join fetch reporte.user",
    ...
    @Query(
        "select distinct reporte from Reporte reporte left join fetch reporte.centro left join fetch reporte.doctor left join fetch reporte.user"
    )
