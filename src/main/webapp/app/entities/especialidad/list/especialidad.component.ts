...
export class EspecialidadComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['nombreEspecialidad'];
  ...
  trackId = (_index: number, item: IEspecialidad): number => this.especialidadService.getEspecialidadIdentifier(item);

  search(query: string): void {
    if (query && EspecialidadComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (EspecialidadComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.especialidads = dataFromBody;
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.especialidadService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
