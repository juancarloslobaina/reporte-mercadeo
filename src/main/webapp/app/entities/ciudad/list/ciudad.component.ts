...
import { EntityArrayResponseType, CiudadService } from '../service/ciudad.service';
...
export class CiudadComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['nombreCiudad'];
  ...
  trackId = (_index: number, item: ICiudad): number => this.ciudadService.getCiudadIdentifier(item);

  search(query: string): void {
    if (query && CiudadComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (CiudadComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.ciudads = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ICiudad[] | null): ICiudad[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.ciudadService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.ciudadService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
