...
import { EntityArrayResponseType, DoctorService } from '../service/doctor.service';
...
export class DoctorComponent implements OnInit {
  ...
  trackId = (_index: number, item: IDoctor): number => this.doctorService.getDoctorIdentifier(item);

  search(query: string): void {
    if (query && DoctorComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (DoctorComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.doctors = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IDoctor[] | null): IDoctor[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.doctorService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.doctorService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
